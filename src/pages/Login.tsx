...
export const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // No need to navigate, OAuth handles the redirect
    } catch (error: any) {
      addToast(error.message, 'error');
      setIsLoading(false);
    }
        ...
        <div className="w-full max-w-sm space-y-10">
          ...
          <div className="space-y-4">
            <button 
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 py-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white px-2 text-gray-400 tracking-widest">Or with email</span>
              </div>
            </div>
          </div>
