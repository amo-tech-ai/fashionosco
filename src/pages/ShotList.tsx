...
export const ShotList: React.FC = () => {
  ...
  const loadAiDraft = () => {
    const saved = localStorage.getItem('latest_wizard_shots');
    if (saved) {
      try {
        const parsed: Shot[] = JSON.parse(saved);
        const mapped: ShotItem[] = parsed.map(s => ({
          id: s.id,
          name: s.name,
          type: 'Photo', // Default
          status: 'Draft',
      ...
      alert("No recent AI draft found in the wizard.");
    }
  };
      ...
      <div className="flex-1 flex flex-col min-h-0">
         <div className="flex justify-between items-center mb-6">
            <div>
               ...
               <p className="text-sm text-[#6B7280]">Summer 2025 Campaign • {shots.length} Shots</p>
            </div>
            <div className="flex space-x-3">
               <button onClick={loadAiDraft} className="flex items-center gap-2 py-2 px-3 bg-purple-50 text-purple-700 text-xs font-bold uppercase tracking-widest rounded-lg border border-purple-100 hover:bg-purple-100 transition-colors shadow-sm">
               ...
               <div className="grid grid-cols-1 gap-4">
                  {shots.map((shot, idx) => (
                     <div 
                        ...
                        className={`bg-white border border-[#E5E5E5] rounded-xl p-4 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-200 group flex gap-4 items-center cursor-default ${draggedIndex === idx ? 'opacity-50 border-dashed border-gray-400' : ''}`}
                     >
                        ...
                        {/* Thumbnail */}
                        <div className="w-16 h-16 bg-[#F7F7F5] rounded-lg border border-[#E5E5E5] flex items-center justify-center text-[#9CA3AF] flex-shrink-0 relative overflow-hidden">
                           {shot.type === 'Video' ? <Video size={20} /> : <ImageIcon size={20} />}
                           {shot.priority && (
                              <div className={`absolute bottom-0 left-0 right-0 h-1 ${
                                 shot.priority === 'High' ? 'bg-red-500' : shot.priority === 'Medium' ? 'bg-yellow-500' : 'bg-gray-300'
                              }`}></div>
                           )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                           <div className="md:col-span-2">
                              ...
                              <p className="text-xs text-[#6B7280] line-clamp-1" title={shot.description}>
                                 {shot.description || `Model: ${shot.model} • Outfit: ${shot.outfit}`}
                              </p>
                           </div>
