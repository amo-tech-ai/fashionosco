...
export const ProductionDesk: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  // Fix: Destructure using correctly named properties returned by useProductionState hook and specify ProductionCue as the generic type
  const { 
    callSheet: cues, 
    ...
    currentTime, 
    stats 
  } = useProductionState<ProductionCue>([], DEFAULT_CUES);
  ...
  const toggleProduction = () => {
    ...
    addToast(isRunning ? "Session Paused" : "Session Live", isRunning ? "info" : "success");
  };
